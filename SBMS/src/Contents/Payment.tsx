...
const FormComponent: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://localhost:8080/api/payments'); // Replace with your API endpoint for schools
      if (response.ok) {
  ...
  const handleSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/payments'); // Replace with your API endpoint
      if (response.ok) {
